...
class CallbackModule(CallbackBase):
    ...
    def set_options(self, task_keys=None, var_options=None, direct=None):
        ...
        if self.FOREMAN_URL.startswith('https://'):
            if not os.environ.get('SATELLITE_USERNAME'):
                self._disable_plugin('SATELLITE_USERNAME %s is not defined')

            if not os.environ.get('SATELLITE_PASSWORD'):
                self._disable_plugin('SATELLITE_PASSWORD %s is not defined') 
